...
%
%
\Sean{Here I am trying to keep notation consistent with the whole quasi- circular inspiral of two point masses example.
I replaced little r with D since it is the distance to the source defined earlier. (Is this true)}
...
\begin{figure}[H]
    ...
    \caption{Visualising a binary black hole system. The two black holes with masses \(M_1\) and \(M_2\) orbit each other in a quasi-circular orbit, 
...
\subsection{Building the Posterior Distribution}
...
In Figure~\ref{fig:Optimising_Hyperparams}, we plot surfaces of the log marginal likelihood across different pairs of hyperparameters. In several panels, we observe relatively flat peaks, indicating that a broad region of the hyperparameter space yields similarly high log likelihood values.
In the final 3D scatter plot, we zoom into the region surrounding the point estimate hyperparameters obtained by the optimisation algorithm. The consistent colouring in this region reinforces the idea that multiple hyperparameter combinations can produce near-optimal log likelihoods. 
This ambiguity or flatness in the likelihood surface is addressed in the following section.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{LatexPlots/1dplots/LogLikeallparams.png}
    \caption{For a GPR with noise where we have the noise as a hyper-parameter we are forced to optimise a length \((\ell\)) hyper-parameter, a variance \(\sigma_f^2\) hyper-parameter and a noise \(\sigma_n^2\) hyper-parameter. 
    ...
    In the final panel we plot a 3-dimensional scatter plot and illustrate the point estimate given by the optimisation algorithm \texttt{optimizer="fmin\_l\_bfgs\_b"} as the black dot. This point is located at
    $(\sigma_f^2 = 1.16, \ell = 0.109 \sigma_n^2 = 0.105)$.}
   \label{fig:Optimising_Hyperparams}
\end{figure} 


\subsection{Quantifying Hyper-parameter Uncertainty}
\label{sec: MCMC}
We have previously discussed the role of kernels and how their hyperparameters influence the shape of both the prior and posterior predictive distributions. As shown in Figure~\ref{fig:Optimising_Hyperparams}, optimising hyperparameters does not necessarily yield a single “best” solution. 
...
\subsection{Multi-Dimensional GPR}
...
\noindent
\subsubsection{Homoscedastic Noise Models}
\begin{itemize}
...
\subsubsection{Heteroscedastic Noise Models}
\begin{itemize}
...
Two final models were selected: one with the overall best test performance, and a second with simpler hyperparameters to serve as a baseline for comparison and to help guard against overfitting. Markov Chain Monte Carlo (MCMC) (explained in Section~\ref{sec: MCMC}) was performed on both models
...
\subsection{Hyper-parameter Uncertainty}
\textbf{MCMC on \texttt{RBFMatern}}
From Figure~\ref{fig:MCMCRBFMatern} we can see that some parameters have single peak distributions with low variance associated to their posterior distribution such as \(\theta_2, \ell_5,\ell_6,\ell_1\). This distributions indicate that 
these hyper-parameters have relatively low uncertainty to them. Other hyper-parameeters such as \(\theta_1\) exhibits a bi-model distribution with large variance. This shows that there is no clear best parameter in this space to maximise the likelihood
and so this paramater space has a high uncertainty to it. 
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{LatexPlots/final_gps_plots/MCMCRBFMat.png}
    \caption{The hyper-parameter posterior of the best \texttt{RBFMatern} model. The model hyper-parameters from Table~\ref{tab:final_gpr_hyperparams} initialise each of the walkers (theres 4*10(dimension number) walkers). For each walker we generate 500 samples and use a burnin of 100 and a thin of 15.
    The resulting parameter distributions are then plotted. The vertical red, blue and green lines indicate peak, mean, and pointwise estimates respectively.}
    \label{fig:MCMCRBFMatern}
\end{figure}
To visualise the difference in distributions from a posterior marginalised over these parameter distrubtions to our point estimate posteriors plotted prevously in Figure~\ref{MCMCvsmarginalised}
we plot the \text{RBFMatern} pointwise model along with the \texttt{RBFMatern} model marginalised over its hyper-parameters. We observe that in all three panels, the MCMC credible regions are noticeably 
wider than the pointwise confidence intervals, especially in regions with sparse or noisy data. 
This reflects the additional uncertainty introduced by accounting for hyperparameter variability. 
In the left and right panels, the wider spread of the blue region highlights locations where the likelihood surface is relatively flat, 
leading to greater variation across plausible hyperparameter samples.
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{LatexPlots/final_gps_plots/MCMCpointwisecomparison_all.png}
    \caption{MCMC marginalised vs pointwise}
    \label{fig:MCMCvsmarginalised}
\end{figure}
Examining Table~\ref{tab:pointwise_vs_mcmc} we see that the pointwise model results in much better
metric scores.
\begin{table}[H]
    \centering
    \small
    ...
    \hline
    \end{tabular}
    \caption{Comparison of regression performance metrics for the RBFMat model using a pointwise (MAP) solution versus the MCMC marginalised model. The MCMC model reflects increased uncertainty, resulting in reduced fit scores but a more robust and probabilistically faithful representation.}
...
%     \caption{The hyper-parameter posterior of the \text{Matnoerr} model. The model hyper-parameters from Table~\ref{tab:final_gpr_hyperparams} initialise the walker. We generate 500 samples and use a burnin of 100 and a thin of 15.
%     The resulting parameter distributions are then plotted. The vertical red, blue and green lines indicate peak, mean, and pointwise estimates respectively.}
...
\newpage
\appendix
\subsection{Derivation of Predictive Distribution}
\label{appendix:A}
