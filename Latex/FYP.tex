...
\maketitle
\Sean{Why is this indenting???}\Sarp{Indent is standard, and looks better in my opinion, but it's your call.}
\begin{abstract}The gold standard for gravitational waveforms (GW) from merging black holes comes from computationally expensive numerical relativity (NR) simulations. While analytic approximations can greatly reduce costs,
    they are not uniformly accurate across parameter space. To address this, Reference~\cite{Ogpaper} introduced a Bayesian framework that selects the most accurate analytic model --- in a given region of the binary black hole parameter space --- based on mismatch values between models and NR. Instead of running dense,
...
\subsection{Plane Wave Solution from General Relativity}
\label{sec:GRintro}
...
%
\paragraph{Learning Noise over the Input Space:}
%
...
and then combine them by multiply them. For example, multiplying RBF kernels defined on each input dimension yields a multi-dimensional RBF kernel.
